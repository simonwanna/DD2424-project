...
DATA_DIR = 'data'
BATCH_SIZE = 64
NUM_EPOCHS = 8  # Set to 5 for fewer runs per experiment
LEARNING_RATE = 1e-5
...
val_dataset = ImageFolder(root=os.path.join(
...
if __name__ == '__main__':
    ...
    for l in range(1, 4):  # Unfreeze more layers progressively, change upper bound for deeper unfreeze
        print(f"\033[93m" + f"Unfreezing {l} layers..." + "\033[0m")
        best_val_acc = 0.0
        ...
        optimizer = torch.optim.Adam(
            # optimizer for unfrozen model
            filter(lambda p: p.requires_grad, model.parameters()), lr=LEARNING_RATE)

        for epoch in range(1, NUM_EPOCHS + 1):
            train_loss, train_acc = run_epoch(
                train_loader, 'train', model, criterion, optimizer)
            ...
            if val_acc > best_val_acc:
                ...
                torch.save(
                    model.state_dict(), f'checkpoints/best_resnet18_finetuned_breed_s1_l={l}.pth')
                print(
                    "\033[92m" + f"Saved best model for l={l} with val acc: {val_acc:.4f}" + "\033[0m")

    print(
        "\033[92m" + f"Training complete. Best val acc: {best_val_acc:.4f}" + "\033[0m")
